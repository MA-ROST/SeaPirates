...
void Ship::draw() const {
	if		( state == State::hovered ) { ofSetColor (ofColor::green); }
	else if ( state == State::paused )  { ofSetColor (ofColor::red); }
	else								{ ofSetColor (255); }

	avatar.draw(coordinate.x, coordinate.y);
}

void Ship::processMouse (int mouseX, int mouseY, bool isMousePressed)
{
	bool isHovered = isShipHovered(mouseX, mouseY);
	if		( isHovered && isMousePressed )	{ state = State::paused; }
	else if ( isHovered )					{ state = State::hovered; }
...
bool Ship::isShipHovered (int mouseX, int mouseY) const
{
	return mouseX > coordinate.x &&
	       mouseX < coordinate.x + avatar.getWidth() &&
	       mouseY > coordinate.y &&
	       mouseY < coordinate.y + avatar.getHeight() ? true : false;
}
...
std::ostream& operator<<(std::ostream& out, const Ship& ship) {
	...
	switch ( ship.state ) {
		case Ship::State::moving : state = "Moving";
			break;
		case Ship::State::hovered : state = "Hovered";
			break;
		case Ship::State::paused : state = "Paused";
			break;
	}
